...
import { SettlementBanner } from '@/components/vendor/SettlementBanner';
import { SettlementModal } from '@/components/vendor/SettlementModal';

function LogoMark() {
    // logoSrc is a static Next.js import — bundled into /_next/static/media/
    // so it works in both web (Vercel CDN) and the Electron standalone server
    // which serves /_next/static/ but not the public/ directory.
    return (
        <Image
            src={logoSrc}
            alt="ByteBusiness POS"
            height={100}
...
function VendorLayoutInner({ children }: { children: React.ReactNode }) {
    ...
    const navItems = [
        ...
        { href: '/vendor/history', label: 'History', icon: <Clock size={14} />, active: pathname === '/vendor/history' },
            ...
            <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
                ...
                <nav className="vendor-topbar flex h-[58px] items-center px-5 gap-4 flex-shrink-0 z-20 w-full">
                    ...
                    <div className="ml-auto flex items-center gap-2 shrink-0">
                        <SettlementBanner onPayClick={() => setSettlementOpen(true)} />
                ...
                <SettlementModal isOpen={settlementOpen} onClose={() => setSettlementOpen(false)} />
            </main>
        </div>
    );
}
