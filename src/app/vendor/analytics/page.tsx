...
import {
    ...
    IndianRupee, CheckCircle2, Clock, X as XIcon, RefreshCw,
} from 'lucide-react';
...
const SETTLEMENT_STATUS: Record<string, { label: string; cls: string; icon: React.ReactNode }> = {
    paid: { label: 'Paid', cls: 'bg-emerald-50 text-emerald-700 border-emerald-100', icon: <CheckCircle2 size={11} /> },
    verification_pending: { label: 'Verifying', cls: 'bg-amber-50 text-amber-700 border-amber-100', icon: <Clock size={11} /> },
    pending: { label: 'Pending', cls: 'bg-orange-50 text-orange-700 border-orange-100', icon: <Clock size={11} /> },
    overdue: { label: 'Overdue', cls: 'bg-red-100 text-red-800 border-red-200', icon: <AlertCircle size={11} /> },
    rejected: { label: 'Rejected', cls: 'bg-red-50 text-red-700 border-red-100', icon: <XIcon size={11} /> },
};
...
export default function VendorAnalytics() {
    ...
    const fetchSettlements = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch('/api/admin/settlements', {
                headers: { Authorization: `Bearer ${idToken}`, 'x-vendor-phone': phoneNumber ?? '' },
            });
            if (res.ok) {
    ...
    return (
        <div className="vendor-workspace h-full overflow-y-auto">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                        {(['today', '7d', '30d'] as RangeKey[]).map(r => (
                            <button
                                key={r}
                                onClick={() => setRange(r)}
                                className={`px-3 py-1.5 rounded-lg text-[12px] font-semibold transition-all ${range === r
                            ...
                            <div className="flex bg-gray-50 p-0.5 rounded-lg border border-gray-100 gap-0.5">
                                {(['revenue', 'orders'] as const).map(m => (
                                    <button
                                        ...
                                        className={`px-3 py-1 text-[11px] rounded-md transition-all capitalize ${chartMode === m
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={topItemsData} layout="vertical"
                                    ...
                                    <Tooltip
                                        ...
                                        formatter={(v) => [`${Number(v ?? 0)} units`, 'Quantity']}
                                    />
            ...
            {/* ── SETTLEMENT HISTORY ─────────────────────────────────── */}
            <div className="mx-5 mb-6 rounded-2xl border border-gray-100 bg-white shadow-sm overflow-hidden">
                <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-50">
                    <div>
                        <h2 className="text-[13px] font-bold text-gray-900">Settlement History</h2>
                        <p className="text-[11px] text-gray-400 mt-0.5">Daily platform fee · ₹2 per online order</p>
                    </div>
                    <div className="flex items-center gap-3">
                        {settlements.length > 0 && (
                            <div className="flex items-center gap-3 text-[11px] text-gray-500">
                                ...
                                <span className="font-bold text-gray-900">
                                    ₹{settlements.filter(s => s.status === 'paid').reduce((s, r) => s + r.payable_amount, 0)} total
                                </span>
                            </div>
                        )}
                        <button
                            ...
                            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[11px] text-gray-500 hover:text-gray-800 hover:bg-gray-50 border border-gray-100 transition-colors"
                        >
                            <RefreshCw size={11} className={settlementsLoading ? 'animate-spin' : ''} />
                            Refresh
                        </button>
                    </div>
                </div>
                ...
                {settlementsLoading && settlements.length === 0 ? (
                    ...
                    <div className="py-10 text-center text-gray-400">
                        <IndianRupee size={24} className="mx-auto mb-2 opacity-30" />
                        <p className="text-[12px] font-medium">No settlements yet</p>
                        <p className="text-[11px] mt-0.5">Generated daily at 9:00 AM</p>
                    </div>
                ) : (
                    <div className="divide-y divide-gray-50">
                        {settlements.map(s => {
                            const cfg = SETTLEMENT_STATUS[s.status] ?? SETTLEMENT_STATUS.pending;
                            const paidAt = s.paid_at ? new Date(s.paid_at).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Kolkata' }) : null;
                            return (
                                <div key={s.id} className="flex items-center gap-4 px-5 py-3 hover:bg-gray-50/60 transition-colors">
                                    {/* Date */}
                                    <div className="w-20 shrink-0">
                                        <p className="text-[12px] font-bold text-gray-900">{s.settlement_date}</p>
                                    </div>
                                    ...
                                    <div className="flex-1 flex items-center gap-4 min-w-0">
                                        <div className="flex items-center gap-1.5 text-gray-500">
                                            ...
                                            <span className="text-[12px] font-semibold">{s.total_online_orders} orders</span>
                                        </div>
                                        <div className="flex items-center gap-1 text-gray-500">
                                            <span className="text-[11px]">×</span>
                                            <span className="text-[12px] font-semibold">₹{s.rate_per_order}</span>
                                        </div>
                                        <div className="flex items-center gap-1 font-bold text-gray-900">
                                            <span className="text-[11px] text-gray-400">=</span>
                                            <span className="text-[13px]">₹{s.payable_amount}</span>
                                        </div>
                                    </div>

                                    {/* UTR */}
                                    {s.transaction_id ? (
                                        <div className="hidden sm:block text-[11px] font-mono text-gray-400 truncate max-w-[120px]" title={s.transaction_id}>
                                            {s.transaction_id}
                                        </div>
                                    ) : (
                                    ...
                                    {/* Paid time */}
                                    <div className="text-[11px] text-gray-400 w-16 text-right shrink-0">
                                        {paidAt ?? '—'}
                                    </div>

                                    {/* Status badge */}
                                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full border text-[10px] font-semibold shrink-0 ${cfg.cls}`}>
                                        {cfg.icon}
                                        {cfg.label}
                                    </span>
                                </div>
...
function KpiCard({ label, value, sub, icon, accent, danger, small }: {
    ...
    return (
        <div className={`rounded-2xl border px-5 py-4 ${accent ? 'bg-indigo-600 border-indigo-500' : 'bg-white border-gray-100'}`}>
            ...
            <p className={`font-bold leading-none truncate ${small ? 'text-[17px]' : 'text-[26px]'} ${accent ? 'text-white' : danger ? 'text-red-500' : 'text-gray-900'
                }`}>
                {value}
            </p>
