...
import { X, ClipboardList, Image as ImageIcon, Banknote, CheckCircle2, Clock, Truck, Package, Send } from 'lucide-react';
...
export default function OrderDetailsDrawer({ isOpen, onClose, order, onAccept, onReject, onShip, onDeliver }: OrderDetailsDrawerProps) {
            ...
            {isOpen && (
                <>
                    ...
                    >
                        ...
                        {/* Scrollable content */}
                        <div className={`flex-1 overflow-y-auto bg-gray-50 ${isActionable || isPreparing || isDispatched ? 'pb-28' : 'pb-6'}`}>
                        ...
                        {/* Ship action — shown for Preparing orders */}
                        {isPreparing && onShip && (
                            <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-4 shadow-[0_-8px_24px_rgba(0,0,0,0.08)] z-20">
                                <button
                                    onClick={handleShip}
                                    disabled={shipping}
                                    className="w-full py-3.5 rounded-xl font-black text-[14px] text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 transition-colors flex items-center justify-center gap-2"
                                >
                                    {shipping ? (
                                        <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Shipping…</>
                                    ) : (
                                        <><Send size={16} /> Ship Order</>
                                    )}
                                </button>
                            </div>
                        )}

                        {/* Deliver action — shown for Dispatched orders */}
                        {isDispatched && onDeliver && (
                            <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-4 shadow-[0_-8px_24px_rgba(0,0,0,0.08)] z-20">
                                <button
                                    onClick={handleDeliver}
                                    disabled={delivering}
                                    className="w-full py-3.5 rounded-xl font-black text-[14px] text-white bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 transition-colors flex items-center justify-center gap-2"
                                >
                                    {delivering ? (
