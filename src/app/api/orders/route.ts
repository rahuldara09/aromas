...
export async function POST(req: NextRequest) {
    ...
    // ── 1b. Settlement lock check — block online orders if vendor hasn't paid ──
    const settingsSnap = await adminDb.doc('settings/storeSettings').get();
    if (settingsSnap.exists && settingsSnap.data()?.settlementLocked === true) {
        return NextResponse.json(
            { error: 'Online ordering is temporarily unavailable. Please try again later.' },
            { status: 503 }
        );
    }
    ...
    try {
        ...
        const orderRef = await adminDb.collection('orders').add({
            ...
            // Per-order platform fee (₹2) — must be paid before vendor can accept
