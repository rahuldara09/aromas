...
export async function POST(req: NextRequest) {
    ...
    const { name, price, categoryId, category, imageURL, onlinePrice, code, serialNumber, onlineSerialNumber, isOnlineItem, isPOSItem, isAvailable, description } = body;
    ...
    try {
        ...
        if (onlinePrice !== undefined) doc.onlinePrice = Number(onlinePrice);
        if (code !== undefined) doc.code = code;
        if (serialNumber !== undefined) doc.serialNumber = Number(serialNumber);
        if (onlineSerialNumber !== undefined) doc.onlineSerialNumber = Number(onlineSerialNumber);
        if (isOnlineItem !== undefined) doc.isOnlineItem = Boolean(isOnlineItem);
        if (isPOSItem !== undefined) doc.isPOSItem = Boolean(isPOSItem);
        if (description !== undefined) doc.description = description;

        const docRef = await adminDb.collection('products').add(doc);
...
export async function PUT(req: NextRequest) {
    ...
    const numericFields = new Set(['price', 'onlinePrice', 'serialNumber', 'onlineSerialNumber']);
    const boolFields = new Set(['isAvailable', 'isOnlineItem', 'isPOSItem']);
    const allowedFields = [
        'name', 'price', 'categoryId', 'category', 'imageURL', 'isAvailable',
        'description', 'onlinePrice', 'code', 'serialNumber', 'onlineSerialNumber',
        'isOnlineItem', 'isPOSItem',
    ];
