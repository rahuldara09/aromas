...
import { ListOrdered, MapPin, LogOut, Filter, Pencil, Check, X, User, Calendar, Mail, Phone, Home, Building } from 'lucide-react';
...
type AddressWithFields = Address & { hostel?: string; room?: string; fullAddress?: string };
...
export default function AccountPage() {
    ...
    const saveEdit = async () => {
        if (!phoneNumber) return;
        if (!editName.trim() || !editHostel.trim() || !editRoom.trim()) {
            toast.error('Please fill in Name, Hostel, and Room Number');
            return;
        }
        setSaving(true);
        try {
            await updateUserAddress(phoneNumber, editName.trim(), editHostel.trim(), editRoom.trim(), editFullAddress.trim());
                                                        ...
                                                        <div className="p-5 bg-gray-50">
                                                            ...
                                                            <div className="space-y-3">
                                                                ...
                                                                <div className="grid grid-cols-2 gap-3">
                                                                    <div>
                                                                        <label className="block text-xs font-medium text-gray-600 mb-1">Hostel Name <span className="text-red-500">*</span></label>
                                                                        <input
                                                                            type="text"
                                                                            value={editHostel}
                                                                            onChange={(e) => setEditHostel(e.target.value)}
                                                                            className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-200 focus:border-red-400 transition bg-white"
                                                                            placeholder="e.g. Gilbreth Hostel"
                                                                        />
                                                                    </div>
                                                                    <div>
                                                                ...
                                                                <div>
                                                                    <label className="block text-xs font-medium text-gray-600 mb-1">Full Address <span className="text-gray-400">(optional)</span></label>
                                                                    <textarea
                                                                        value={editFullAddress}
                                                                        onChange={(e) => setEditFullAddress(e.target.value)}
                                                                        rows={2}
                                                                        className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-200 focus:border-red-400 transition bg-white resize-none"
                                                                        placeholder="Wing, Floor, Nearby landmark, Additional directions"
                                                                    />
                                                                </div>
                                                                {/* Action buttons */}
...
function ProfileTab({ user, userProfile, onSignOut }: { user: any, userProfile: UserProfile | null, onSignOut: () => void }) {
    ...
    const [fullAddress, setFullAddress] = useState(userProfile?.lastFullAddress || '');
    const [saving, setSaving] = useState(false);
    ...
    useEffect(() => {
        if (userProfile) {
            ...
            setFullAddress(userProfile.lastFullAddress || '');
        } else {
    ...
    const handleSave = async () => {
        ...
        if (!isValidPhone) return toast.error('Enter a valid 10-digit phone number');
        if (!birthday) return toast.error('Date of birth is required');
        if (!hostel.trim()) return toast.error('Hostel name is required');
        ...
        try {
            ...
            setUserProfile({ ...userProfile, ...updatedData, phone: formattedPhone, totalOrders: userProfile?.totalOrders ?? 0 });
    ...
    return (
        <div className="p-4 md:p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <FloatingInput label="Email Address" value={email} readOnly icon={Mail} />
            ...
            <div className="border-t border-gray-100 pt-4 space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FloatingInput label="Hostel Name *" value={hostel} onChange={(e: any) => setHostel(e.target.value)} icon={Building} />
                ...
                <div className="relative">
                    <textarea
                        ...
                        rows={2}
                        placeholder="Wing, Floor, Nearby landmark, Additional directions"
                        className="w-full border border-gray-100 hover:border-gray-200 rounded-xl px-4 pt-5 pb-2 text-sm text-gray-900 focus:outline-none focus:ring-4 focus:ring-red-50 focus:border-red-500 transition-all resize-none bg-white"
                    />
                    <label className="absolute left-4 top-1.5 text-[10px] font-bold uppercase tracking-wider text-gray-400">
                        Full Address (optional)
                    </label>
                </div>
                {/* Fixed location fields */}
                <div className="grid grid-cols-3 gap-3">
                    <FloatingInput label="City" value="Mumbai" readOnly icon={MapPin} />
                    <FloatingInput label="State" value="Maharashtra" readOnly icon={MapPin} />
                    <FloatingInput label="Pincode" value="400087" readOnly icon={MapPin} />
                </div>
            </div>
