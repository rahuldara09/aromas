...
export async function getProductsByCategory(categoryId: string): Promise<Product[]> {
    if (!isFirebaseConfigured()) return MOCK_PRODUCTS.filter((p) => p.categoryId === categoryId && p.isOnlineItem === true);
    try {
        const q = query(collection(db, 'products'), where('categoryId', '==', categoryId), where('isOnlineItem', '==', true));
        ...
        return MOCK_PRODUCTS.filter((p) => p.categoryId === categoryId && p.isOnlineItem === true);
    }
}
...
export function listenToOrder(orderId: string, callback: (order: Order | null) => void): () => void {
    ...
    return onSnapshot(ref, (snap) => {
        ...
        console.error('[listenToOrder] snapshot error:', error.code, error.message);
    });
}
...
export async function getUserByPhone(phone: string): Promise<UserProfile | null> {
    ...
    try {
        ...
        return {
            ...
            lastFullAddress: data.lastFullAddress ?? '',
...
export async function getUserAddresses(phone: string): Promise<Address[]> {
    ...
    try {
        ...
        return [{
            ...
            city: 'IIM Mumbai, Powai',
            pincode: '400087',
            hostel: data.lastHostel ?? '',
            room: data.lastRoom ?? '',
            fullAddress: data.lastFullAddress ?? '',
...
export async function updateUserAddress(
    ...
    await setDoc(userRef, {
        ...
        lastHostel: hostel,
        lastRoom: room,
        ...(fullAddress !== undefined ? { lastFullAddress: fullAddress } : {}),
...
export async function upsertUserProfile(
    ...
    try {
        ...
        const baseData = {
            ...
            lastHostel,
            lastRoom,
            ...(lastFullAddress !== undefined ? { lastFullAddress } : {}),
            updatedAt: Timestamp.now(),
            ...extraFields
        };
