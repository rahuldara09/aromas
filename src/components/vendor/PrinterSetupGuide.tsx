...
export default function PrinterSetupGuide({ isOpen, onClose }: PrinterSetupGuideProps) {
    ...
    const steps = {
        mac: {
            folderPath: getPath('/Users/<your-username>/Documents/byte_printer'),
            cdCommand: getPath('cd ~/Documents/byte_printer'),
            pm2Commands: `npm install -g pm2\npm2 start server.js --name byte_printer\npm2 startup\npm2 save`,
            downloadLinks: [
                { label: 'Download .dmg (Recommended)', href: '/downloads/byte_printer_mac.dmg', primary: true },
                { label: 'Download .zip (Alternative)', href: '/downloads/byte_printer_mac.zip', primary: false }
            ]
        },
        windows: {
            folderPath: getPath('C:\\Users\\<your-username>\\Documents\\byte_printer'),
            cdCommand: getPath('cd %USERPROFILE%\\Documents\\byte_printer'),
            pm2Commands: `npm install -g pm2 pm2-windows-startup\npm2 start server.js --name byte_printer\npm2-startup install\npm2 save`,
            downloadLinks: [
                { label: 'Download for Windows (.zip)', href: '/downloads/byte_printer_win.zip', primary: true }
            ]
        }
    };
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white dark:bg-gray-900 w-full max-w-2xl max-h-[90vh] overflow-hidden rounded-[24px] shadow-2xl flex flex-col border border-gray-100 dark:border-gray-800">
                ...
                <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-white/50 dark:bg-gray-900/50 backdrop-blur-md sticky top-0 z-10">
                    <div>
                        <div className="flex items-center gap-2">
                            <h2 className="text-xl font-black text-[#111827] dark:text-white leading-tight">BytePrinter Setup</h2>
                            {isConnected && (
                        ...
                        <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mt-1">BytePrinter v4.1.0 • 3-Level Setup Guide</p>
                    </div>
                    ...
                    <div className="border-t border-gray-100 dark:border-gray-800 pt-6">
                        ...
                        {showAdvanced && (
                            <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4 pb-4">
                                ...
                                <AdvancedStep
                                    title="View Logs"
                                    command="pm2 logs byte_printer"
                                    description="Debug failures in real-time."
                                />
                                <AdvancedStep
                                    title="Control PM2"
                                    command="pm2 restart byte_printer"
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-900/50 border-t border-gray-100 dark:border-gray-800 flex justify-between items-center px-8">
                    <p className="text-[9px] font-black text-gray-400 uppercase tracking-tight">BytePrinter v4.1.0 • PRO MODAL</p>
